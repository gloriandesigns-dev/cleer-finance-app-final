...
import { Home, List, Plane, Sliders, Plus, Receipt, DollarSign, CreditCard as CreditCardIcon, X, Camera, Image as ImageIcon } from 'lucide-react-native';
...
export const MainLayout: React.FC<MainLayoutProps> = ({ children, activeTab, disableFab }) => {
  const router = useRouter();
  const [fabVisible, setFabVisible] = useState(false);
  const [sheetMode, setSheetMode] = useState<'main' | 'receipt'>('main');
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setFabVisible(false);
      router.push({ pathname: '/receipt-form', params: { imageUri: result.assets[0].uri } });
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.navWrapper}>
        ...
        <TouchableOpacity 
          style={[styles.fab, disableFab && styles.fabDisabled]} 
          activeOpacity={0.8}
          onPress={openFab}
          disabled={disableFab}
        >
          <Plus size={28} color={disableFab ? Colors.textSecondary : Colors.primaryBlack} />
        </TouchableOpacity>
      </View>
          ...
          <Pressable style={styles.bottomSheet}>
            {sheetMode === 'main' ? (
              <>
                ...
                <TouchableOpacity style={styles.sheetAction} onPress={() => setSheetMode('receipt')}>
                ...
                <TouchableOpacity style={styles.sheetAction} onPress={() => { setFabVisible(false); router.push('/request-funds'); }}>
                  <View style={styles.sheetIconContainer}>
              ...
              <>
                <View style={styles.sheetHeader}>
                  ...
                  <TouchableOpacity onPress={() => setSheetMode('main')} style={styles.closeButton}>
                    <X size={24} color={Colors.textSecondary} />
                  </TouchableOpacity>
                </View>
                
                <TouchableOpacity style={styles.sheetAction} onPress={() => { setFabVisible(false); router.push('/camera'); }}>
                  <View style={styles.sheetIconContainer}>
                ...
                <TouchableOpacity style={styles.sheetAction} onPress={pickImage}>
                  <View style={styles.sheetIconContainer}>
